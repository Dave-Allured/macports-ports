...
#
# Note:  As of 2025 May 27, this version 1.2 is a drop-in replacement
#   for 1.1,except that C++ flags are added to configure.cxxflags,
#   rather than to configure.cppflags.
#
#   Discussion: https://github.com/macports/macports-ports/pull/25895
#
#   New public symbols:
#    * legacysupport::get_cxx_flags
#    * MACPORTS_LEGACY_SUPPORT_CXXFLAGS
#
# Usage:
#
...
proc legacysupport::get_cxx_flags {} {
    global os.platform os.major prefix
    if {${os.platform} eq "darwin" && ${os.major} <= [option legacysupport.newest_darwin_requires_legacy]} {
        if { [option legacysupport.disable_function_wrap] } {
            return "-isystem${prefix}/include/LegacySupport -D__DISABLE_MP_LEGACY_SUPPORT_REALPATH_WRAP__=1 -D__DISABLE_MP_LEGACY_SUPPORT_SYSCONF_WRAP__=1"
...
proc legacysupport::add_legacysupport {} {
    global prefix os.platform os.major
    global ls_cache_incpath ls_cache_ldflags
    global ls_cache_cppflags ls_cache_cxxflags
    ...
    if { ${os.platform} eq "darwin" && ${os.major} <= [option legacysupport.newest_darwin_requires_legacy] } {
        ...
        set ls_cache_cxxflags "[legacysupport::get_cxx_flags]"
